...
<template>
  <article class="bg-gray-800 border border-gray-700 rounded-2xl shadow-md p-6 hover:shadow-lg hover:border-gray-600 transition-all duration-300 flex flex-col gap-4">
    
    <!-- Encabezado: Avatar + Nombre + Estado -->
    <header class="flex items-center gap-4">
      <!-- Avatar -->
      <div
        class="w-14 h-14 rounded-full bg-indigo-600 flex items-center justify-center text-white font-bold text-xl flex-shrink-0"
      ...
      <div class="flex-1 min-w-0">
        ...
        >
          <span
            ...
            :class="user.isActive ? 'bg-green-400' : 'bg-gray-500'"
          />
    ...
    <footer class="pt-2 border-t border-gray-700">
      <button
        type="button"
        class="w-full py-2 px-4 bg-indigo-600 hover:bg-indigo-500 text-white
